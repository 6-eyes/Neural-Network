...
impl Matrix {
    ...
    pub fn map(&self, func: impl Fn(f64) -> f64) -> Self {
        Self {
            data: self.data.iter().map(|&x| func(x)).collect::<Vec::<f64>>(),
            rows: self.rows,
            cols: self.cols,
        }
    }
...
impl From<Matrix> for Vec<Vec<f64>> {
    fn from(value: Matrix) -> Self {
        value.data.chunks(value.cols).map(|chunk| chunk.to_vec()).collect::<Vec<Vec<f64>>>()
    }
}
...
impl From<Vec<Vec<f64>>> for Matrix {
    fn from(value: Vec<Vec<f64>>) -> Self {
        ...
        Self {
            rows,
            cols,
            data: value.into_iter().flatten().collect::<Vec<f64>>(),
        }
    }
}
